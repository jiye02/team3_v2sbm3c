...
<c:set var="file1saved" value="${galleryVO.file1saved.toLowerCase() }" />
...
<DIV class='title_line'><A href="./list_by_exhino_search_paging.do?exhino=${exhino }" class='title_link'>${exhiVO.name }</A> > ${title } 삭제</DIV>
...
<DIV class='content_body'>
  <ASIDE class="aside_right">
    ...
    <span class='menu_divide' >│</span>
    <A href="./list_by_exhino_search_paging.do?exhino=${exhino }">기본 목록형</A>    
    <span class='menu_divide' >│</span>
    <A href="./list_by_exhino_grid.do?exhino=${exhino }">갤러리형</A>
    <span class='menu_divide' >│</span>
    <A href="./update_text.do?galleryno=${galleryno}">수정</A>
    <span class='menu_divide' >│</span>
    <A href="./update_file.do?galleryno=${galleryno}">파일 수정</A>  
  </ASIDE> 
  ...
  <DIV style="text-align: right; clear: both;">  
    <form name='frm' id='frm' method='get' action='./list_by_exhino.do'>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' 
                     onclick="location.href='./list_by_exhino.do?exhino=${exhino}&word='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
        ...
        <DIV style='text-align: center; width: 50%; float: left;'>
          ...
          <c:choose>
            <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}">
              <img src="/gallery/storage/${file1saved }" style='width: 90%;'> 
            </c:when>
