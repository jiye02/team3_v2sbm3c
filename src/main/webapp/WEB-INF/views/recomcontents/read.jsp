...
<c:set var="file1saved" value="${recomcontentsVO.file1saved }" />
<c:set var="thumb1" value="${recomcontentsVO.thumb1 }" />
<c:set var="content" value="${recomcontentsVO.content }" />
<c:set var="map" value="${recomcontentsVO.map }" />
<c:set var="youtube" value="${recomcontentsVO.youtube }" />
<c:set var="word" value="${recomcontentsVO.word }" />
<c:set var="size1_label" value="${recomcontentsVO.size1_label }" />
...
<DIV class='title_line'><A href="./list_by_recno.do?recno=${recno }" class='title_link'>${recomplaceVO.recname }</A></DIV>
...
<DIV class='content_body'>
  <ASIDE class="aside_right">
    ...
    <c:if test="${sessionScope.admin_id != null }">
      ...
      <A href="./update_text.do?recontno=${recontno}&now_page=${param.now_page}&word=${param.word }">글 수정</A>
      <span class='menu_divide' >│</span>
      <A href="./update_file.do?recontno=${recontno}&now_page=${param.now_page}">파일 수정</A>  
      <span class='menu_divide' >│</span>
      <A href="./map.do?recno=${recno }&recontno=${recontno}">지도</A>
      <span class='menu_divide' >│</span>
      <A href="./youtube.do?recno=${recno }&recontno=${recontno}">Youtube</A>
      <span class='menu_divide' >│</span>
      <A href="./delete.do?recontno=${recontno}&now_page=${param.now_page}&recno=${recno}">삭제</A>  
      <span class='menu_divide' >│</span>
    </c:if>

    <A href="javascript:location.reload();">새로고침</A>
    <span class='menu_divide' >│</span>    
    <A href="./list_by_recno.do?recno=${recno }&now_page=${param.now_page}&word=${param.word }">기본 목록형</A>    
    <span class='menu_divide' >│</span>
    <A href="./list_by_recno_grid.do?recno=${recno }&now_page=${param.now_page}&word=${param.word }">갤러리형</A>
  </ASIDE> 
        ...
        <DIV style="width: 100%; word-break: break-all;">
          <c:choose>
            ...
            <c:otherwise> <!-- 기본 이미지 출력 -->
              <img src="/recomcontents/images/none1.png" style='width: 50%; float: left; margin-top: 0.5%; margin-right: 1%;'> 
            </c:otherwise>
          </c:choose>
      ...
      <li class="li_none">
        <DIV>
          <c:if test="${file1.trim().length() > 0 }"> <%-- ServletRegister.java: registrationBean.addUrlMappings("/download"); --%>
            첨부 파일: <a href='/download?dir=/recomcontents/storage&filename=${file1saved}&downname=${file1}'>${file1}</a> (${size1_label})  
          </c:if>
        </DIV>
      </li>   
    </ul>
