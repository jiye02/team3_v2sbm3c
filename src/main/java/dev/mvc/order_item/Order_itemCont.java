...
public class Order_itemCont {
  ...
  /**
   * 예약 결제/회원별 목록
   * http://localhost:9093/order_item/list_by_memberno.do 
   * @return
   */
  ...
  public ModelAndView list(HttpSession session, @RequestParam(value = "order_payno", required = false) Integer order_payno) {
    ModelAndView mav = new ModelAndView();
  ...
  public ModelAndView delete(HttpSession session, @RequestParam(value="order_itemno", defaultValue="0") int order_itemno, Integer order_payno ) {
    ModelAndView mav = new ModelAndView();
    
    this.order_itemProc.delete(order_itemno);      
    mav.setViewName("redirect:/order_item/list.do?order_payno="+order_payno);
