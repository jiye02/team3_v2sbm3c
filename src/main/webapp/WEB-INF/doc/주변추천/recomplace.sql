...
/* Table Name: 카테고리 */
/**********************************/
DROP TABLE recomplace;

commit;

CREATE TABLE recomplace(
    recno                  NUMBER(10)		 NOT NULL	PRIMARY KEY,
    recname                   VARCHAR2(30)		 NOT NULL,
...
COMMENT ON TABLE recomplace is '전시회 주변 추천';
COMMENT ON COLUMN recomplace.recno is '추천 번호';
COMMENT ON COLUMN recomplace.recname is '추천 이름';
COMMENT ON COLUMN recomplace.cnt is '관련 자료수';
COMMENT ON COLUMN recomplace.rdate is '등록일';
COMMENT ON COLUMN recomplace.udate is '수정일';
COMMENT ON COLUMN recomplace.seqno is '출력 순서';
COMMENT ON COLUMN recomplace.visible is '출력 모드';

DROP SEQUENCE recomplace_seq;

CREATE SEQUENCE recomplace_seq
  START WITH 1         -- 시작 번호
  INCREMENT BY 1       -- 증가값
...
-- CREATE
INSERT INTO recomplace(recno, recname, cnt, rdate, seqno) VALUES(recomplace_seq.nextval, '카페', 0, sysdate, 0);

commit;

-- SELECT LIST
SELECT recno, recname, cnt, rdate, seqno, visible FROM recomplace ORDER BY recno ASC;
